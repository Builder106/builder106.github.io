  ...
  return (
    <main className="sr-only" aria-label="Olayinka David Vaughan — portfolio content">
      ...
      <p>
        ...
        2026 at Wesleyan University. Based in Middletown, Connecticut. Interactive 3D portfolio
        built with React Three Fiber, Blender, and custom GLSL shaders.
      </p>
      ...
      <section aria-labelledby="contact-heading">
        ...
        <ul>
          ...
          <li>
            Resume: <a href="/Olayinka_Vaughan_Resume.pdf">Download PDF</a>
          </li>
        </ul>
      </section>
      ...
      <section aria-labelledby="projects-heading">
        ...
        {projects.map(p => (
          <article key={p.id} aria-labelledby={`project-${p.id}-heading`}>
            ...
            <p>{p.blurb}</p>
            <p>
              Cluster: {p.cluster}.{p.stack.length > 0 && <> Stack: {p.stack.join(', ')}.</>}
            </p>
            {(p.links.live || p.links.repo) && (
              <ul>
                {p.links.live && (
                  <li>
                    Live: <a href={p.links.live}>{p.links.live}</a>
                  </li>
                )}
                {p.links.repo && (
                  <li>
                    Repository: <a href={p.links.repo}>{p.links.repo}</a>
                  </li>
                )}
              </ul>
            )}
