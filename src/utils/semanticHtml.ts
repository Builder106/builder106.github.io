...
import { CLUSTER_DISPLAY, projects, type ProjectCluster } from '../data/projects.ts';
...
const CLUSTER_ORDER: ProjectCluster[] = ['quant', 'swe', 'analyst', 'security', 'ml'];
...
export function buildSemanticContentHTML(): string {
  ...
  function renderProject(p: (typeof projects)[number]): string {
    const stack = p.stack.length > 0 ? `<p>Stack: ${escape(p.stack.join(', '))}.</p>` : '';
    const links: string[] = [];
    if (p.links.live) {
...
export function buildLlmsTxt(): string {
  ...
  for (const cluster of CLUSTER_ORDER) {
    ...
    for (const p of inCluster) {
      const url = p.links.live ?? p.links.repo ?? `${SITE_URL}/#project-${p.id}`;
      const repoNote = p.links.live && p.links.repo ? ` (source: ${p.links.repo})` : '';
